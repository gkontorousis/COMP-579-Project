...
def main():
    parser = argparse.ArgumentParser("RLStock full pipeline: tune → train → trade")
    ...
    parser.add_argument("--n_trials", type=int, default=5,
                        help="Optuna trials (0 to skip tuning)")
    parser.add_argument("--timesteps_per_trial", type=int, default=3_000,
                        help="Training timesteps per tuning trial")
    ...
    if args.n_trials > 0:
        ...
        best_params = rl_algos.tune(
            DDPG, "RLStockTrain-v0", "RLStockValidation-v0",
            args.timesteps_per_trial, args.seed, args.n_trials,
        )
    ...
    rl_algos.train(DDPG, train_env, args.timesteps, args.seed, args.model_out,
                   sigma=sigma, **best_params)
    train_env.close()
    ...
    rl_algos.trade_online(DDPG, args.model_out, test_env, online_model_out)
    test_env.close()
