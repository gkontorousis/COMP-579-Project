...
Following Liu et al.\ \cite{liu2018practical}, we model the stock-trading problem as a finite-horizon Markov decision process (MDP). At each trading day, the agent observes the current portfolio state, chooses buy/sell/hold actions for each stock, and receives a reward based on the resulting change in portfolio value. In our implementation, the MDP is deterministic with respect to the fixed historical price series: once the dataset split and the agent's action are fixed, the next state is fully determined.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{pics/markov.png}
...
\]
The environment executes all sell orders before buy orders. Sell actions are processed from most negative to least negative, while buy actions are processed from largest positive to smallest positive. This ordering increases the cash available for later buy orders and mirrors the behavior of the original reference environment.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\linewidth]{pics/ddpg_model_test.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\linewidth]{pics/td3_model_test.png}
