...
Each episode corresponds to one complete pass through a selected date split. The data are divided using the same periods as the original paper: training from 2009--2014, validation during 2015, and trading from 2016 to 2018. As in the original paper, the agent is allowed to continue updating its policy during the trading phase, making this an online trading evaluation rather than a purely fixed-policy test. The episode terminates when the environment reaches the final trading day of the current split.
...
We performed Bayesian hyperparameter optimization with Optuna \citep{akiba2019optuna}, using 10 trials per algorithm. After selecting hyperparameters, each model was trained for 30 episodes and evaluated across three random seeds (42, 43, 44); results are reported as mean and standard deviation across seeds. Additional optimization details are provided in Appendix~\ref{app:hpo_details}.
...
This section will provide the full Optuna setup, including search spaces, objective definitions, and trial configuration details.
